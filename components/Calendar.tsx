...
function Week({ week, month, db, setDate, activeDate }) {
	//row in the calendar
	return (
	...
	return (
		<LinkIf href={href} isLink={!db && isWorkout}>
			<div
				...
				title={workout?.style || 'No workout'}
			>
...
export default function Calendar({
	...
	useDate: [date: Date, setDate: React.Dispatch<React.SetStateAction<Date>>];
	month?: boolean;
	week?: boolean;
	db?: boolean;
}) {
	...
	const calendar = date
		...
		.filter(
			//if its month view, use every week
			(week) =>
				month ||
				week.some(
					//otherwise only use the week containing the date in state
					(day) =>
	...
	if (month) {
		...
		const yearDisplay = calendar[0].map(
			//add -YY to string, more customizable than doing in conjunction with later display
