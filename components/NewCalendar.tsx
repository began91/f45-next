...
interface WeekType {week: WorkoutType[], month?: boolean, db: boolean, activeDate: Date}

function Week({ week, month, db, activeDate}: WeekType) {//row in the calendar
    return (
        <div className={styles.week}>
            {week.map((workout, i) => (
                <DaySquare
                    ...
                    month={month}
                    db={db}
                    isActiveDate={workout.month-1 === activeDate.getMonth() && workout.date === activeDate.getDate()}
...
function DaySquare({ workout, month, db , isActiveDate, isThisMonth}: DaySquareType) {
...
export default function Calendar({
    ...
    week,//week view
    db,//db update page
}: {date: Date, month?: WorkoutType[][], week?: WorkoutType[], db?: boolean}) {
        ...
        if (month) {//for month view, simply display the month of the date in state, including year.
            ...
            const monthDisplayShort = week.map(
                (workout:WorkoutType)=>{
    ...
    const backDate = week ? new Date(date.getFullYear(),date.getMonth(),date.getDate()-7) : new Date(date.getFullYear(),date.getMonth()-1, date.getDate())
    const fwdDate = week ? new Date(date.getFullYear(),date.getMonth(),date.getDate()+7) : new Date(date.getFullYear(),date.getMonth()+1, date.getDate())
    const hrefBack = `/${week ? "weekly":"schedule"}/${backDate.getFullYear()}/${backDate.getMonth()+1}/${backDate.getDate()}` 
    const hrefFwd = `/${week ? "weekly":"schedule"}/${fwdDate.getFullYear()}/${fwdDate.getMonth()+1}/${fwdDate.getDate()}`
    const today = new Date();
    const hrefToday = `/${week ? "weekly":"schedule"}/${today.getFullYear()}/${today.getMonth()+1}/${today.getDate()}`
        ...
        <div className="calendar">
            ...
            {month?.map((week, i) => (
                <Week
                    ...
                    activeDate={date}
                />
            )) || (<Week week={week} db={db} activeDate={date}/>)}
        </div>
    );
}
